...
def record_info(info,filename,mode):
    ...
    if mode =='test':
        result = (
              ...
              'Prec@1 {top1} '
              'Prec@5 {top5} \n'.format( batch_time=info['Batch Time'],top1=info['Prec@1'], top5=info['Prec@5']))      
        print (result)
        df = pd.DataFrame.from_dict(info)
...
def record_info_b(info_b,filename,mode):
    ...
    if mode =='train':
        ...
        result = (
              ...
              .format(videoname=info_b['videoname'],
               output_pred=info_b['output_pred'],label=info_b['label'],max_scores=info_b['max_scores']))      
        

        df = pd.DataFrame.from_dict(info_b)
        column_names = ['videoname','output_pred','max_scores','label']
    ...
    if mode =='test':
        result = (
              ...
              'Loss {loss} '
              'Prec@1 {top1} '
              'Prec@5 {top5} \n'.format( batch_time=info_b['Batch Time'],
               loss=info_b['Loss'], top1=info_b['Prec@1'], top5=info_b['Prec@5']))      
        print (result)
        df = pd.DataFrame.from_dict(info_b)
